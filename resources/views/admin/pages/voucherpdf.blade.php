...
<html>
<head>
    <title>{{$singleVoucher->voucherid}} pdf file</title>
    <style>
        ...
        <div class="row">
            ...
            <div class="col-sm-6 order-sm-0"> <strong>Company Details</strong>
                <address>
                    @if(!empty($sitedata))
                        {{ $sitedata->company }}<br>
                        {{ str_replace(',Nepal','',$sitedata->location) }}<br>
                        P.O. Box: {{ $sitedata->pobox }}<br>
                        Nepal
                    @endif
                </address>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        @if(!empty($singleVoucher))
                            <tr>
                                <td><span class="text-3">{{ $singleVoucher->voucherid  }}</span></td>
                                <td class="text-center">{{ \App\Officecategory::find($singleVoucher->category_id)->name }}</td>
                                <td class="text-center">{{ strip_tags($singleVoucher->description) }}</td>
                                <td class="text-center">{{ strip_tags($singleVoucher->narrative)  }}</td>
                                <td class="text-right">Npr. {{ $singleVoucher->price  }}</td>
                            </tr>
                            <span class="hidden">{!! $total += $singleVoucher->price !!}</span>
                        @endif
                        <tr>
