              ...
              @if (!empty($flash))
                ...
                <div class="ps-product ps-product--inner">
                  <div class="ps-product__thumbnail">
                    ...
                    <img src="{{ product_img($image->image) }}" alt="{{$recent_products->product->slug}}">
                    </a>
            ...
            @if (!empty($women_fashion))
              @foreach ($women_fashion as $women_products)
                  ...
                  <div class="ps-product">
                        <div class="ps-product__thumbnail"><a href="{{ route('single-product', $women_products->slug) }}">
                          <img src="{{ $product_image ? $product_image : asset('/images/noimage.png')}}" alt="{{$women_products->slug}}"></a>
                          @if ($discount > 0)
                        ...
                        <a class="ps-product__vendor" href="#">Jojayo Store</a>
                          ...
                          <div class="ps-product__content hover">
                            <a class="ps-product__title"  href="{{ route('single-product', $women_products->slug) }}">{{ $women_products->name }}</a>
                            <p class="ps-product__price sale">NPR {{ $total_price }}
                                @if ($discount > 0)
                                    <del>NPR {{ $total_price + $discount }}</del>
                                @endif
                              ...
                              <div class="ps-product">
                                  <div class="ps-product__thumbnail"><a
                                          href="{{ route('single-product', $men_products->slug) }}"><img
                                              src="{{ $product_image ? $product_image : asset('/images/noimage.png')}}" alt="{{$men_products->slug}}"></a>
                                      @if ($discount > 0)
                                          <div class="ps-product__badge">-{{ $discount }} NPR </div>
                                      @endif
                                      <ul class="ps-product__actions">
          ...
          <div class="ps-container">
              ...
              <div class="ps-section__content">
                  <div class="ps-carousel--nav owl-slider" data-owl-auto="false" data-owl-loop="false"
                      ...
                      data-owl-item-xs="2" data-owl-item-sm="2" data-owl-item-md="3" data-owl-item-lg="4"
                      data-owl-item-xl="6" data-owl-duration="1000" data-owl-mousedrag="on">
                      @if (!empty($kid_fashion))
                          @foreach ($kid_fashion as $kid_products)
                              @php
                              //$product_image =
                              //
                              (isset($kid_products->images)&&count($kid_products->images)>0)?$kid_products->images[0]->image:'';
                              $starting_price = App\Models\ProductSize::where('product_id', $kid_products->id)->first();
                              $discount = $starting_price['discount'];
                              $total_price = $starting_price['selling_price'] - $discount;
                              $product_image =
                              (count($kid_products->images)>0)?product_img($kid_products->images[0]['images'][0]['image']):'';
                              @endphp
                              <div class="ps-product">
                                  <div class="ps-product__thumbnail"><a
                                          href="{{ route('single-product', $kid_products->slug) }}"><img
                                              src="{{ $product_image ? $product_image : asset('/images/noimage.png')}}" alt="$kid_products->slug"></a>
                                      @if ($discount > 0)
                                          <div class="ps-product__badge">-{{ $discount }} NPR </div>
                                      @endif
                                      <ul class="ps-product__actions">
              ...
              @foreach ($latest_products as $recent_products)
                  ...
                  <div class="col-xl-3 col-lg-4 col-md-4 col-sm-6 col-12 ">
                      <div class="ps-product--horizontal">
                        <div class="ps-product__thumbnail"><a href="{{ route('single-product', $recent_products->slug) }}">
